...
#### A climatological targeting recommendation for rapid-intensification (RI) observations in the Gulf of America
...
| **Date** | 21 June 2026 *(revised 22 June 2026; corrected 13–14 August 2026)* |
...
Rapid intensification — a large, fast increase in a hurricane's wind speed over a single day — is the most difficult part of hurricane forecasting and among the most dangerous situations for coastal communities, because it can turn a modest storm into a major one with little warning. The Gulf of America is where this occurs most frequently in the United States.
...
The analysis also used reconstructed ocean temperature data to test whether the depth of warm water beneath a storm predicts rapid intensification. In individual major storms, deep warm water was present during the periods of fastest intensification. This matches a known physical mechanism: a hurricane cools the sea surface as it passes, and that cooling reduces the storm's intensity. When the warm layer is deep, the storm cannot mix down to cooler water below it, so the cooling effect is weaker and intensification can continue [4][6].

Across the 76 Gulf hurricanes of the ocean-reanalysis era, 1994 to 2025, this relationship was weak when water depth was measured directly beneath the storm. The sample included 698 storm observations recorded every six hours, 89 of which marked the onset of rapid intensification. At onset, the median water depth was 46 meters, compared with 44 meters elsewhere, a difference too small to rule out chance. Adding water depth to a statistical model that already accounted for a storm's location, the season, and its current intensity produced almost no gain in skill. The reason is that water depth and location carry overlapping information: the deepest warm water in the Gulf is consistently found in the same region and season, the southern Gulf in late summer, which is also where and when rapid intensification is most common.

A storm's own passage cools the water directly beneath it, so measuring depth at the storm's center understates the warm water actually available to fuel it. Measuring the same water about one degree ahead of the storm, before the storm reaches it, removes that bias and reveals a real signal. There, deeper water separates onset from non-onset locations far more clearly (p about 1 in 100,000, versus p about 0.07 at the center), and adding that ahead-of-track depth to the same statistical model produces a genuine, if modest, gain in skill beyond location, season, and intensity. The practical implication is where to measure: ocean-heat sampling ahead of an approaching storm carries information a center-sampled reading does not.
...
The analysis draws on HURDAT2 (1851–2025), comprising 2,004 Atlantic systems at 6-hourly best-track resolution [10], restricted to the 326 systems that both reached hurricane strength and entered a Gulf-of-America polygon, with Caribbean- and Atlantic-only systems excluded by a hand-drawn coastline mask. An RI onset is identified at any fix whose wind rises by at least 30 knots over the following 24 hours, taking the nearest fix in a 21-to-27-hour window, the standard threshold [1]; the onset is located at the starting fix, the place and time at which one would want eyes on the ocean before the storm intensifies. To distinguish genuine preference from mere traffic, the Gulf domain (18–31 °N, 80–98 °W) is gridded at 0.25°, and each cell records both its onsets and all hurricane fixes passing through, so that the quantity of interest is propensity — onsets divided by traffic — rather than where storms happen to travel. A Gaussian kernel-density estimate of the onset points, with a 0.75° bandwidth, yields a smooth likelihood surface and the nested core and watch boxes; the lift reported throughout is the conditional probability of an onset given a hurricane fix inside a box, relative to the domain average. Because the core and watch boxes are drawn from the same density surface whose lift is then reported inside them, the 3.1× and 1.95× figures are in-sample and biased high by construction; an out-of-sample check, for example scoring a held-out period against boxes drawn from an earlier one, would be needed before treating the exact lift values as a fixed operational benchmark rather than a directional result.
...
Across the full ocean-reanalysis-era population, 76 Gulf hurricanes, 698 storm observations recorded every six hours, 89 of them RI onsets, that signal weakens sharply. Warm-water depth does separate onset fixes from non-onset fixes, but only faintly: median D26 of 46 versus 44 meters, a Mann-Whitney *p* of about 0.07, and an area under the ROC curve of 0.55, where 0.5 is no skill. Taken alone, depth is at best a marginal discriminator. The more telling test is not whether depth carries *any* signal, but whether it carries *independent* signal: does it sharpen prediction once the obvious predictors are already in hand? It does not. In a cross-validated model, adding D26 to latitude, longitude, season, and current intensity leaves out-of-sample skill essentially unchanged (AUC 0.693 to 0.689). Deep warm water genuinely fuels intensification in individual storms. Across the population, however, its information is already contained in *where* and *when* the storm occurs. The southern Gulf in late summer is precisely where the deep warm water lies.[^1]

![Scaled D26-vs-RI test](figures/fig4_scaled_d26.png)
*__Figure 6. Does deep warm water mark where storms start to rapidly intensify?__ (76 Gulf hurricanes, 1994–2025.) Hurricane records log each storm's position and strength every 6 hours; this uses 698 of those snapshots, 89 of which mark the start of rapid intensification. __(A)__ Where rapid intensification began, the warm water was only slightly deeper than where it didn't (typically 46 vs 44 m) — so warm-water depth on its own is barely better than a coin flip at telling the two apart. __(B)__ Going from the shallowest to the deepest warm water, the rate of rapid intensification rises only a little — and adds nothing once you already know the storm's location, the time of year, and the storm's current intensity. __Bottom line:__ location and season already tell you where the deep warm water is (Section 5.7).*
...
**A direct check of the cold-wake caveat.** The center-sampled test above is conservative by construction: an intensifying storm sits in its own cold wake, so D26 measured at the storm center understates the warm water that actually fueled it. Re-sampling the *same* ocean fields a short distance ahead of each storm along its heading, the un-waked water it is about to enter, sharpens the signal markedly. About one degree ahead, the median warm-layer depth separates to 52 meters at RI-onset fixes versus 42 meters elsewhere (Mann-Whitney *p* ≈ 1×10⁻⁵, against *p* ≈ 0.07 at the center). The area under the ROC curve for depth alone rises from 0.55 to 0.64. The decisive change is that D26 now adds a small but genuine *independent* increment of skill beyond location, season, and intensity (cross-validated AUC +0.01 to +0.02, versus essentially zero at the center). The effect is stable across lead distances from half a degree to two degrees. Removing the cold-wake and position noise recovers the very ocean-heat signal the center sample had masked. This sharpens rather than softens the conclusion: measured ocean heat *is* informative when sampled where it matters, ahead of the storm, and this is precisely where real-time observing effort should go (Section 6).
...
5. Scharroo, R., W. H. F. Smith, and J. L. Lillibridge, 2005: Satellite altimetry and the intensification of Hurricane Katrina. *Eos, Transactions American Geophysical Union*, **86**(40), 366–367. doi:10.1029/2005EO400004
    ...
    *Annotation.* Attributes Hurricane Katrina's intensification to the Loop Current's dynamic topography (from altimetry) rather than sea-surface temperature alone — a conclusion contested at the time by Sun et al. (2006, *Eos* 87(8)) with a Scharroo reply, though independently corroborated here by Jaimes & Shay (2009) [6]. Supports the Loop Current attribution (§5.4; §5.5).
...
17. Beven, J. L., II, L. Alaka, and C. Fritz, 2025: *Hurricane Milton (AL142024) Tropical Cyclone Report*, 5–10 October 2024. NOAA/National Hurricane Center, Miami, FL, issued 31 March 2025. https://www.nhc.noaa.gov/data/tcr/AL142024_Milton.pdf
    ...
    *Annotation.* Official post-storm report; documents Hurricane Milton's extreme rapid intensification to Category 5 (minimum central pressure below 900 mb) after forming in the southwestern Gulf of America off Tampico, Mexico (the report's Bay of Campeche RI event, §5.4, occurred later along the storm's track, not at genesis). Source for the Milton case (§5.4; §5.7). Byline and 31 March 2025 issuance date confirmed from the report cover page.
...
19. Benedetto, K. M., and J. C. Trepanier, 2020: Climatology and spatiotemporal analysis of North Atlantic rapidly intensifying hurricanes (1851–2017). *Atmosphere*, **11**(3), 291. doi:10.3390/atmos11030291
...
20. Yaukey, P. H., 2014: Intensification and rapid intensification of North Atlantic tropical cyclones: the role of geography, time of year, age since genesis, and storm characteristics. *International Journal of Climatology*, **34**(4), 1038–1049. doi:10.1002/joc.3744
    ...
    *Annotation.* Finds the highest mean intensification rates in the North Atlantic occur in the Gulf of Mexico (as the paper terms it) and Caribbean Sea. Independent corroboration of the southern-Gulf hotspot (§5.6).
...
21. Chassignet, E. P., H. E. Hurlburt, O. M. Smedstad, G. R. Halliwell, P. J. Hogan, A. J. Wallcraft, R. Baraille, and R. Bleck, 2007: The HYCOM (HYbrid Coordinate Ocean Model) data assimilative system. *Journal of Marine Systems*, **65**(1–4), 60–83. (https://www.hycom.org/attachments/191_Chassignet_et_al_07.pdf)
    ...
    *Annotation.* The model underlying the U.S. Navy/NRL HYCOM GOFS 3.1 reanalysis used here to reconstruct D26 for storms from 1994 through 2015, including Katrina and Rita (2005). Data accessed via tds.hycom.org (GLBv0.08 / expt 53.X). Used in §5.7 and the ocean pipeline.
...
22. Lellouche, J.-M., and Coauthors, 2021: The Copernicus Global 1/12° oceanic and sea ice GLORYS12 reanalysis. *Frontiers in Earth Science*, **9**, 698876. doi:10.3389/feart.2021.698876
    ...
    *Annotation.* The Copernicus Marine GLORYS12 reanalysis (product GLOBAL_MULTIYEAR_PHY_001_030) used to reconstruct D26 for storms from 2016 through 2021 and the 76-storm population test; the companion Global Ocean Physics Analysis and Forecast product (GLOBAL_ANALYSISFORECAST_PHY_001_024) supplies 2022 through 2025. Used in §5.7 and the ocean pipeline.

23. NOAA Office of Satellite and Product Operations (OSPO), 2025: *Suspension of All Defense Meteorological Satellite Program (DMSP) Data by July 31, 2025* (1 July 2025) and *DMSP Data Distribution Extended Beyond July 31, 2025* (30 July 2025). NOAA/NESDIS/OSPO operational messages. https://www.ospo.noaa.gov/data/messages/2025/07/MSG_20250701_1815.html ; https://www.ospo.noaa.gov/data/messages/2025/07/MSG_20250730_0015.html

    *Annotation.* Paired agency notices: the first announced suspension of DMSP/SSMIS passive-microwave data distribution to NOAA by 31 July 2025; the second, issued 30 July 2025, extended distribution on a reduced best-effort (8×5) basis rather than terminating it outright. Basis for the passive-microwave coverage-gap concern (§7.3) — corrected here (13 August 2026) to reflect the extension rather than the original, superseded cutoff.

*All citations verified June 2026. Both items previously flagged as open are now resolved: the Milton Tropical Cyclone Report byline [17] was confirmed from the report cover page (Beven, Alaka, and Fritz; issued 31 March 2025), and the passive-microwave coverage-gap concern (§7.3) is now supported by the NOAA/OSPO DMSP discontinuation notice [23]. Corrected 13 August 2026 after a pre-external-review pass: citation [19]'s co-author (Trepanier, not Mercer) and citation [23]'s DMSP status (extended, not terminated, as of 30 July 2025) — see the RapidWatch project record for the full check.*
...
[^1]: This center-sampled test is conservative: an intensifying storm sits in its own cold wake, so D26 at the center understates the warm water that fueled it, and best-track position error adds further noise, both biasing the measured association low. Re-sampling the same fields ahead of the track (the un-waked ocean the storm is entering) confirms this directly, recovering a significant and modestly independent D26 signal; see the ahead-of-track check at the end of Section 5.7. A separate, independent replication of this ahead-of-track effect in the companion `gulf-ri-model` project (a different 1998–2023 sample) also finds a significant signal, at its own p-value. The two tests use different samples and should not be quoted as the same number.
